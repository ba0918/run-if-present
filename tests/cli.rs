...
fn rejects_an_empty_wrapper_command() {
    ...
    assert!(String::from_utf8_lossy(&output.stderr).contains("command must not be empty"));
}
...
fn rejects_an_empty_path_guard() {
    ...
    assert_eq!(output.status.code(), Some(2));
    assert!(output.stdout.is_empty());
    assert!(String::from_utf8_lossy(&output.stderr).contains("path must not be empty"));
}
...
fn rejects_an_empty_chdir_value() {
    ...
    assert_eq!(output.status.code(), Some(2));
    assert!(output.stdout.is_empty());
    assert!(String::from_utf8_lossy(&output.stderr).contains("chdir must not be empty"));
}
